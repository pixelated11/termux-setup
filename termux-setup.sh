...
echo "Now installing required packages..."
sleep 1
pkg install neovim -y && pkg install wget -y && pkg install curl -y && pkg install git -y && pkg install proot-distro -y && pkg install sudo -y && pkg install tsu -y 
&& pkg install neofetch -y && pkg install zip -y
&& pkg install unzip -y && pkg install python2 -y && pkg install python -y && pkg install nodejs -y && pkg install nodejs-lts -y &&
...
while true; do
  ...
  case $yn in
    [Yy]* ) echo "Ok!"; echo "Thanks for using termux setup v1.5"; break;;
